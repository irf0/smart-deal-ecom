...
import { Minus, Plus, Trash2, ShoppingCart, ImageOff, ChevronLeft, Tag, X, CheckCircle2, ChevronDown, ChevronUp } from 'lucide-react'
...
export default function CartPage() {
    ...
    useEffect(() => {
        ...
        if (totalPrice < appliedCoupon.min_cart_value) {
            ...
            setCouponError(`Coupon removed — cart total dropped below ₹${appliedCoupon.min_cart_value.toLocaleString('en-IN')}`)
        }
    }, [totalPrice])
    ...
    function handleApplyCoupon() {
        ...
        startCouponTransition(async () => {
            ...
            if (!match) {
                ...
                if (!directMatch) {
                    setCouponError('Invalid coupon or cart doesn\'t meet the minimum value.')
                    return
                }
        ...
        <div className="min-h-screen bg-gray-50">
            <div className="max-w-3xl mx-auto px-4 py-6">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <h1 className="text-2xl font-bold text-gray-900">
                            Your Cart <span className="text-gray-400 font-medium text-lg">({totalItems} {totalItems === 1 ? 'item' : 'items'})</span>
                        </h1>
                    </div>
                ...
                <div className="flex flex-col gap-3 mb-6">
                    {items.map(item => (
                        <div key={item.variant_id} className="bg-white rounded-2xl border border-gray-200 p-3 flex gap-3 items-center">
                            <Link href={`/product/${item.slug}`} className="shrink-0">
                                <div className="w-20 h-20 rounded-xl bg-gray-100 overflow-hidden flex items-center justify-center">
                                    {item.image_url ? (
                                        <img src={item.image_url} alt={`${item.brand} ${item.model}`} className="w-full h-full object-cover" />
                                    ) : (
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-4 mb-4">
                    ...
                    {eligibleCoupons.length > 0 && !appliedCoupon && (
                        <div className="mb-3">
                            ...
                            >
                                <span>🎉 {eligibleCoupons.length} coupon{eligibleCoupons.length > 1 ? 's' : ''} available for your cart</span>
                                {showEligible ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                            </button>
                            {showEligible && (
                                    ...
                                    {eligibleCoupons.map(c => (
                                        ...
                                        >
                                            ...
                                            <span className="text-gray-500 text-xs">
                                                ...
                                                {c.max_discount_amount ? ` (up to ₹${c.max_discount_amount.toLocaleString('en-IN')})` : ''}
                                            </span>
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-green-600">
                                        {appliedCoupon.discount_percent}% off — you save ₹{discountAmount.toLocaleString('en-IN')}
                                    </p>
                                </div>
                            </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-4 sticky bottom-4">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-gray-500 text-sm">Subtotal ({totalItems} {totalItems === 1 ? 'item' : 'items'})</span>
                        <span className="text-sm font-semibold text-gray-800">₹{totalPrice.toLocaleString('en-IN')}</span>
                    </div>
                    ...
                    {appliedCoupon && (
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-green-600 text-sm font-semibold">−₹{discountAmount.toLocaleString('en-IN')}</span>
                        </div>
                    )}
