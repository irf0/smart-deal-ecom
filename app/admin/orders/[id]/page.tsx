...
export default async function OrderDetailPage({ params }: Props) {
    ...
    return (
        <div className="max-w-3xl mx-auto py-8 px-4 space-y-6">
            ...
            <Link href="/admin/orders" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 transition-colors">
                ...
                <div className="md:col-span-2 space-y-6">
                    ...
                    <Section title="Customer">
                        <div className="rounded-lg border p-4 space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground font-mono">{order.customer_whatsapp}</p>
                        </div>
                    </Section>
                    ...
                    <Section title={`Items (${items.length})`}>
                        <div className="rounded-lg border divide-y overflow-hidden">
                            ...
                            <div className="flex justify-between items-center px-3 py-2.5 bg-gray-50">
                                ...
                                <span className="font-bold">₹{order.total_price.toLocaleString('en-IN')}</span>
                            </div>
                        </div>
                    </Section>
...
function OrderItem({ item }: { item: any }) {
    ...
    return (
        <div className="flex items-center gap-3 p-3">
            {thumbnail ? (
                <img src={thumbnail} className="w-12 h-12 object-cover rounded border shrink-0" />
            ) : (
            ...
            <div className="flex-1 min-w-0">
                <p className="font-medium text-sm">{product?.brand} {product?.model}</p>
                {identifier && (
                    <p className="text-xs text-muted-foreground font-mono mt-0.5">IMEI/S/N: {identifier}</p>
                )}
            </div>
            <p className="font-medium text-sm shrink-0">₹{item.price.toLocaleString('en-IN')}</p>
        </div>
    )
}