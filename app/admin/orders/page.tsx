...
export default async function OrdersPage({ searchParams }: Props) {
    ...
    const counts = (statusCounts ?? []).reduce<Record<string, number>>((acc, o) => {
    ...
    if (filters.q) {
        ...
        query = query.or(`customer_name.ilike.${term},customer_whatsapp.ilike.${term},order_number.ilike.${term}`)
    }
            ...
            <div className="bg-white rounded-lg border overflow-hidden">
                <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th className="text-left font-bold px-4 py-3 text-gray-600">Total</th>
                        ...
                        {orders?.map(order => (
                            ...
                            >
                                ...
                                <td className="px-4 py-3">
                                    <Badge
                                        variant="outline"
                                        className={ORDER_STATUS_STYLES[order.status as OrderStatus] ?? ''}
                                    >
                ...
                {totalPages > 1 && (
                    <div className="border-t px-4 py-3 flex items-center justify-between text-sm">
                        ...
                        <OrderPageNav page={page} totalPages={totalPages} filters={filters} />
                    </div>
                )}
            </div>
        </div>
...
function OrderPageNav({
    ...
    return (
        <div className="flex gap-2">
            {page > 1 && (
                <Link href={href(page - 1)} className="px-3 py-1 rounded border text-sm hover:bg-gray-50">
                    ← Prev
                </Link>
            )}
            {page < totalPages && (
