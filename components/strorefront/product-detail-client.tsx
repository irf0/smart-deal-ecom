                    ...
                    {images.map((img, i) => (
                        <button
                            ...
                            className={`shrink-0 w-16 h-16 rounded-xl overflow-hidden border-2 transition-colors ${i === active
                                ? 'border-accent'
                                : 'border-transparent hover:border-accent-hover'
                                }`}
                        >
                            <img
                                src={img.url}
...
function VariantSelector({
    ...
    const sortedVariants = [...variants].sort((a, b) => {
        const rankDiff =
            (conditionRank[b.condition] ?? 0) -
            (conditionRank[a.condition] ?? 0)
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center justify-between px-4 py-3 rounded-xl border-2 transition-colors text-left ${isSelected
                                ...
                                : 'border-gray-200 hover:border-accent-hover bg-surface-light'
                                }`}
                        >
                            ...
                            <div className="flex items-center gap-2 shrink-0 ml-3">
                                <div className="text-right">
                                    <p className={`font-bold text-base ${isSelected ? 'text-accent' : 'text-gray-900'}`}>
            ...
            {hardwareSpecs.length > 0 && (
                <div className="grid grid-cols-2 gap-2">
                    {hardwareSpecs.map(spec => (
                        <div key={spec.label} className="bg-surface border border-gray-200 rounded-xl px-3 py-2.5">
    ...
    return (
        <div className="min-h-screen bg-surface bg-gray-50">
            <div className="max-w-5xl mx-auto px-4 py-6">
                <Link
                    href="/product"
                    className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-accent transition-colors mb-6"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <button
                            ...
                            className={`w-full flex items-center justify-center cursor-pointer gap-2 text-white font-semibold rounded-2xl py-3.5 transition-colors ${addedToCart
                                ? 'bg-green-600'
