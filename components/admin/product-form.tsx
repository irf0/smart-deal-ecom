...
export default function ProductForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Upload new files to storage first (storage still uses browser client)
      const folderId = product?.id ?? uuidv4();
      const uploadedImages: { url: string; position: number }[] = [];
      for (let i = 0; i < newFiles.length; i++) {
        const file = newFiles[i];
        const ext = file.name.split(".").pop();
        const path = `${folderId}/${uuidv4()}.${ext}`;
  ...
  return (
    <>
      {loading && (
        <Loader text={isEdit ? "Updating product..." : "Adding product..."} />
      )}
      <form onSubmit={handleSubmit} className="space-y-8 max-w-2xl">
          ...
          <div className="space-y-1">
            <Label>
              ...
              <span className="text-xs text-gray-400">
                (free-text summary, e.g. "Midnight Black, Face ID")
              </span>
            </Label>
            <Textarea
              value={specs}
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-gray-400">
            Used for filtering on the storefront. Leave blank if not applicable.
          </p>
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-gray-400">
            Add a row for each grade variant you have in stock.
          </p>
                ...
                <div className="grid grid-cols-12 gap-2 items-end">
                  <div className="col-span-6 space-y-1">
                    ...
                    <Select
                      value={row.condition}
                      onValueChange={(val) =>
                        updateVariantRow(index, "condition", val)
                      }
                    >
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateVariantRow(index, "stock_count", e.target.value)
                      }
                      className="h-8 text-xs"
                      min={0}
                    />
