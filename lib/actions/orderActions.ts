...
export async function createOrder(input: CreateOrderInput) {
    ...
    if (input.couponCode) {
        ...
        const { data: coupon, error } = await supabase
            .from('coupons')
            .select('id, code, discount_percent, min_cart_value, max_discount_amount, usage_limit, times_used, active, valid_from, valid_until')
            .eq('code', code)
            .single()
        ...
        if (coupon.max_discount_amount !== null) {
            validatedDiscount = Math.min(validatedDiscount, coupon.max_discount_amount)
        }
        ...
        if (incrementError) {
            throw new Error(`Coupon "${code}" could not be applied. Please try again.`)
        }
    }
    ...
    if (orderError || !order) {
        ...
        if (validatedCouponCode) {
            await supabase.rpc('decrement_coupon_times_used', { coupon_code: validatedCouponCode })
        }
